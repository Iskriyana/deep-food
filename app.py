...
if activate_vision:
    ...
    st.write("Loading Model... This might take a while, relax!")
    loaded_model = load_model(
    model_path,
    custom_objects=None,
    compile=True
)
    st.write("Model compiled!! Now I can make some predictions")
    time.sleep(0.5)
    st.write("This won't take too long, hope you are not too hungry :D")
    time.sleep(1)
    st.write("I got you!")
    time.sleep(0.5)
    st.write('Deep-foodie is predicting')
...
if st.button('GIVE ME THE RECIPE'):
# The model for the recipe
    st.write('Let me think of what we could prepare today')
    time.sleep(2)
    st.write('Okay, I will give you 3 recipes, but do not ask me for more...')
    time.sleep(1)
    st.write('Feeling tired already :P, Enjoy!')
